    ...
    <n-card title="已注册用户管理" :bordered="false" class="rounded-16px shadow-sm">
      <n-space class="pb-12px" justify="space-between">
        <n-space>
          ...
          <n-button type="primary" @click="handleAddTable"> <icon-uil:search class="mr-4px text-20px" />搜索 </n-button>
        ...
        <NSpace justify={'center'}>
          ...
          <NPopconfirm onPositiveClick={() => handleDeleteTable(row.id)}>
            {{
              default: () => '确认删除',
              trigger: () => (
                <NButton type="error" size={'small'} disabled={row.userRole == Role}>
                  删除
                </NButton>
              )
            }}
...
function Add(data) {
  if (data[0].userRole === '超级管理员') {
    data[0].userRole = '1';
  } else if (data[0].userRole === '管理员') {
    data[0].userRole = '2';
  } else {
    data[0].userRole = '3';
  }
